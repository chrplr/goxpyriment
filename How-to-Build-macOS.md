...
> **Note on SDL3:** `goxpyriment` uses `github.com/Zyko0/go-sdl3` which embeds pre-built SDL3 and SDL3_ttf dylibs directly in the Go binary for both Intel and Apple Silicon. **You do not need to install SDL3 via Homebrew or any other method.** The only prerequisite is Go 1.25+.
...
> Note: SDL3 and SDL3_ttf are embedded in the Go binary via `github.com/Zyko0/go-sdl3/bin/binsdl` and `binttf`. No SDL3 installation is required.
