...
Four things must match between the two suites or the numbers are not comparable.

**Display mode.** `-w` = windowed, `-d N` = display index — the same meaning as in
the Go binary. These used to be reversed here (`-d` meant "windowed"), which
silently turned a fullscreen comparison into a windowed one; windowed mode is
compositor-throttled and its timing is not meaningful. Always measure fullscreen.

**Garbage collector.** By default the cyclic collector is suspended with
`gc.disable()`, mirroring Go's `debug.SetGCPercent(-1)`. Pass `--gc` to leave it
running. Run each test twice, with and without, on both suites:
...
Note the asymmetry, which belongs in any write-up: `gc.disable()` stops CPython's
*cyclic* collector only — reference counting keeps freeing objects and cannot be
switched off — whereas Go's tracing collector is suspended outright.

**Audio device and buffer.** `--audio-frames` maps to the PTB backend's
`blockSize`, the counterpart to the Go binary's `-audio-frames`. Set it, or the
`tones` and `av` results characterise the local buffer configuration rather than
PsychoPy.

`--audio-device N` selects which speaker device to open; `--sysinfo` lists the
indices. **On PsychoPy 2026.1.x this, not `--audio-lib`, is the effective
control**: that release routes every sound through its `SpeakerDevice` /
PsychToolbox layer and ignores `prefs.hardware['audioLib']` for stream creation —
verified here, where `ptb`, `sounddevice` and `pygame` all produced the identical
PsychToolbox error. The `--audio-lib` flag is kept for older PsychoPy versions.

If no stream can be opened the script exits non-zero with an explanation rather
than a traceback. **Record the device index used**, because audio numbers only
compare across machines that opened the same kind of device.

**Configuration capture.** Run `python timing_tests.py --sysinfo` on every machine
and keep the output beside the results, as with the Go binary's `-sysinfo`.
...
|---|---|
| `python timing_tests.py --test jitter -w` | `Timing-Tests -test jitter -w` |
| `python timing_tests.py --test frames --cycles 120` | `Timing-Tests -test frames -cycles 120` |
| `python timing_tests.py --test flash --frames-on 1 --frames-off 59` | `Timing-Tests -test flash -frames-on 1 -frames-off 59` |
| `python timing_tests.py --test av --soa-ms 0` | `Timing-Tests -test av -soa-ms 0` |
| `python timing_tests.py --test square --period-ms 100` | `Timing-Tests -test square -period-ms 100` |
| `python timing_tests.py --test sound --cycles 300` | `Timing-Tests -test sound -cycles 300` |
| `python timing_tests.py --test rt --cycles 60` | `Timing-Tests -test rt -cycles 60` |
| `python timing_tests.py --test drain` | `Timing-Tests -test drain` |
...
```
python timing_tests.py --test jitter --duration-s 30 -w
```
...
Presents a bright phase of `--frames-on` frames (`--level-b`, default 255)
followed by a dark phase of `--frames-off` frames (`--level-a`, default 0), for
`--cycles` cycles.  A trigger pulse is sent at the first frame of each bright
phase (requires DLP-IO8-G; skipped otherwise).

The two phases are independent, matching the Go binary.  Two statistics are
reported, both against the **measured mean** rather than a nominal target
derived from `--hz`, so a wrong assumed refresh rate cannot skew them:

- `Bright-phase duration` — first dark flip − first bright flip
- `Period` — bright onset − previous bright onset
...
### `flash` — alias for `frames`

The single-frame flash case is simply `--frames-on 1`, so `flash` is an alias
for `frames` rather than a separate implementation — as in the Go binary.

```
python timing_tests.py --test flash --frames-on 1 --frames-off 59 --cycles 60
```
...
```
python timing_tests.py --test av --soa-ms 0 --freq-hz 1000 --tone-ms 50 --iti-ms 1000 --cycles 30 -w
```
...
python timing_tests.py --test sound --cycles 30 --iti-ms 450 -w
```
...
```
python timing_tests.py --test rt --cycles 60 --iti-ms 1000 -w
```
...
| `--frames-on` | frames | 2 | Frames per dark/bright phase |
| `--frames-off` | flash | 60 | Dark frames between flashes |
