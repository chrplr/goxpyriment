...
| `eyetracker/` | Gaze tracking — vendor-neutral `Tracker` interface, a socket client for trackers behind a native SDK (EyeLink via `bridge/eyelink_bridge.py`, Tobii Pro via `bridge/tobii_bridge.py`, transport shared in `bridge/bridgelib.py`), and a mouse-driven simulator. See `eyetracker/CLAUDE.md` |
