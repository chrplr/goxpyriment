...
// stimulus onset; stimuli.PresentStreamOfStimuliHooks takes an OnsetCallback
// fired post-flip, and the triggers/ package has the device backends, but
...
func main() {
	...
	exp.Run(func() error {
		...
		for i := range rows {
			...
			events, timing, perr := stimuli.PresentStreamOfStimuliFunc(
				exp.Screen, elems, 0, 0,
				func(stimuli.FrameContext) error {
					if !*noCrosshair {
